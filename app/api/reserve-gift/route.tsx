...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: updatedGift, error: updateError } = await supabaseAdmin
      .from("gifts")
      .update({
        status: "reserved",
    ...
    if (updateError || !updatedGift) {
      return NextResponse.json(
        { error: "Gift not available or not found" },
        { status: 409 }
      );
    }
    ...
    try {
      ...
      await notifyOwner(
        ...
        `<p>${name} (${email}) reservou o presente: <strong>${
          updatedGift.name
        }</strong>${viaPix ? " (via Pix)" : ""}</p>`
      );
    } catch (emailError) {
